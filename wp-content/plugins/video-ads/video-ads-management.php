...
class video_ads
{
	...
	function register_configuration(){
		...
		$ads_config = new Options_Page('ads_config',
									array(
										...
										'version'    => 'Cactus - Video Ads 3.2.5',
										),
									array(
										'submit_text' => 'Save'
										)
	...
	function add_advs_to_video($atts, $content = "")
	{
		...
		if($enable_ads != '' && $enable_ads == 'yes')
		{
			...
			if(isset($ads_ids_arr) && count($ads_ids_arr) > 1)
			{
				...
				$playback_ads_ids 			= implode(',', $ads_ids_arr);

				$playback_ads = $wpdb->get_results("select id from " . $wpdb->prefix . "posts inner join " . $wpdb->prefix ."postmeta on " . $wpdb->prefix ."posts.id = " . $wpdb->prefix . "postmeta.post_id where id in(" . $playback_ads_ids  . ") and post_status='publish' and post_type='video-advs' and " . $wpdb->prefix ."postmeta.meta_key = 'advs_expiry_date' and " . $wpdb->prefix ."postmeta.meta_value >= NOW() order by rand() limit 1");
				...
				else
				{
					...
					if($is_mobile_or_tablet == false)
		    		{
						if($video_url != '')
						{
							...
							else if(strpos($video_url, site_url()) !== false)
							{
								$data_ads = $video_url;
				...
				else
				{
					...
					if($is_mobile_or_tablet == false)
		    		{
						if($pb_video_url != '' && $pb_cactus_advs_type == 'video')
						{
							...
							else if(strpos($pb_video_url, site_url()) !== false)
							{
	...
	*/
	...
	{
		echo '<style type="text/css" media="screen">.error, .updated {display:none;}</style>';
		echo '<div class="error"><p></p></div>';
		echo '<div class="updated"><p></p></div>';
	}
