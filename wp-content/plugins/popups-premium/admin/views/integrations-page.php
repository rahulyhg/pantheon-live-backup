...
redirect_uri=<?php echo urlencode( 'https://timersys.com/popups/oauth/');?>" data-provider="ccontact" class="button-primary" target="_blank" onclick="ccontact(jQuery(this)); return false;">Authorize with Constant Contact</a>
		...
		<div class="js-newsletter collapse-div">
			<table class="form-table">
				<tr valign="top" class="">
					<th colspan="2"><h3><?php _e( 'The newsletter plugin', $this->plugin_slug ); ?> <a href="#" class="toggle-provider"><i class="spu-icon spu-icon-angle-down"></i></a></h3></th>
				</tr>
				<tr valign="top" class="">
					<th><label for="ajax_mode"><?php _e( 'Api Key', $this->plugin_slug ); ?></label></th>
					<td colspan="3">
						<label><input type="text" id="nl_api" name="spu_integrations[newsletter][nl_api]" value="<?php echo @$opts['newsletter']['nl_api'];?>" class="regular-text <?php echo !$nl_connected?:'spu_license_valid';?>" /> <?php
							if(! $nl_connected ){
								echo '<button class="enable-api">Connect</button>';
							}?>
							<p class="help"><?php _e( 'Get your <a href="http://local.dev/test/wp-admin/admin.php?page=newsletter_main_main#tabs-advanced" target="_blank">Newsletter API key from advanced settings panel</a>', $this->plugin_slug ); ?></p>
					</td>
				</tr>
			</table>
			<?php include_once('partials/nl-lists.php');?>
		</div>
		<?php /*
	...
	*/?>
		<div class="js-activecampaign collapse-div">
			<table class="form-table">
				<tr valign="top" class="">
					<th colspan="2"><h3><?php _e( 'ActiveCampaign', $this->plugin_slug ); ?> <a href="#" class="toggle-provider"><i class="spu-icon spu-icon-angle-down"></i></a></h3></th>
				</tr>
				<tr valign="top" class="">
					<th><label for="ajax_mode"><?php _e( 'Api Url', $this->plugin_slug ); ?></label></th>
					<td colspan="3">
						<label><input type="text" id="ac_url" name="spu_integrations[activecampaign][ac_url]" value="<?php echo @$opts['activecampaign']['ac_url'];?>" class="regular-text <?php echo !$ac_connected?:'spu_license_valid';?>" />
					</td>
				</tr>
				<tr valign="top" class="">
					<th><label for="ajax_mode"><?php _e( 'Api Key', $this->plugin_slug ); ?></label></th>
					<td colspan="3">
						<label><input type="text" id="ac_api" name="spu_integrations[activecampaign][ac_api]" value="<?php echo @$opts['activecampaign']['ac_api'];?>" class="regular-text <?php echo !$ac_connected?:'spu_license_valid';?>" /><?php
							if(!$ac_connected){
								echo '<button class="enable-api">Connect</button>';
							}?>
							<p class="help"><?php _e( 'Log into your ActiveCampaign settings page, then on left menu click on "developer" and copy API key and url fields', $this->plugin_slug ); ?></p>
					</td>
				</tr>
			</table>
			<?php include_once('partials/ac-lists.php');?>
		</div>
