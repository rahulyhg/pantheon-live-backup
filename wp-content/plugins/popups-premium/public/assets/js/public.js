...
(function($){
	...
	$(".spu-box").each(function() {
		...
		// move to parent in safe mode or top bar mode
		if( spuvar.safe_mode || $box.hasClass('spu-top-bar') || $box.hasClass('spu-bottom-bar') ){
		...
		if ( ! advancedClose && ! $box.after_content ) {
			...
			$('body').on(event, function (ev) {
				...
				if( ev.originalEvent !== undefined && ! $(ev.target).is('.spu-clickable') && ! $box.hasClass('spu-top-bar') && ! $box.hasClass('spu-bottom-bar') ) {
        ...
        if( box_form.length ) {
            ...
            $box.on('submit','form:not(".newsletter-form, .wpcf7-form, .gravity-form, .infusion-form, .spu-disable-ajax, .spu-optin-form, .widget_wysija, .ninja-forms-form, .flp_form")', function(e){
                ...
                var submit 	= true,
                    form 		= $(this),
                    data 	 	= form.serialize(),
                    url  	 	= form.hasClass('mc4wp-form') ? window.location.href : form.attr('action'),
	...
	function facebookFix( box ) {
        ...
        var checkConvertedRules = function ( $box ) {
            ...
            for( var i = 0 ; i < c_popups.length ; i++ ) {
                if( !spuReadCookie('spu_box_' + c_popups[i]) )
                    show_popup = false;
            }

            for( var i = 0 ; i < nc_popups.length ; i++ ) {
                if( spuReadCookie('spu_box_' + nc_popups[i]) )
                    show_popup = false;
            }
