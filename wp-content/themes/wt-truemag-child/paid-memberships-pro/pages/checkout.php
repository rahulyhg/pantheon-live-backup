...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<thead>
		<tr>
			<th>
				<span class="pmpro_thead-name"><?php printf(__('You have selected <strong>%s</strong>.', 'pmpro'), $pmpro_level->name);?></span>
				<?php if( $pmpro_levels ) { ?><span class="pmpro_thead-msg"><a href="<?php echo pmpro_url("levels"); ?>"><?php _e('change', 'pmpro');?></a></span><?php } ?>
			</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			...
			<!-- 	</p> -->
				...
				<div id="pmpro_level_cost">
					...
					<?php } ?>
					<?php echo pmpro_getLevelCost($pmpro_level, true, true); ?>
					<?php echo pmpro_getLevelExpiration($pmpro_level); ?>
                                </div>

				<?php do_action("pmpro_checkout_after_level_cost"); ?>
                        
                      <!-- code by virendra to remove discount code function if level cost is zero.-->
                      <?php $wt_temp = explode('INR', pmpro_getLevelCost($pmpro_level, true, true));
                      $wt_temp = explode('now', $wt_temp['1']);
                       $wt_temp = explode('.', $wt_temp['0']); 
                       $wt_temp_cost = intval($wt_temp['0']);
                      if ( $wt_temp_cost === 0){$pmpro_show_discount_code = false;} ?>
                      

                                <?php //back to original code
                                if($pmpro_show_discount_code) { ?>

					<?php if($discount_code && !$pmpro_review) { ?>
						<p id="other_discount_code_p" class="pmpro_small"><a id="other_discount_code_a" href="#discount_code"><?php _e('Change discount code', 'pmpro');?></a>.</p>
					<?php } elseif(!$pmpro_review) { ?>
						<p id="other_discount_code_p" class="pmpro_small"><?php _e('If you have a Discount Code:', 'pmpro');?> <a id="other_discount_code_a" href="#discount_code"><?php _e('Click here to apply', 'pmpro');?></a>.</p>
...
<?php        
        //code from preheaders folder checkout.php to set multiple gateways set valid gateways - the active gateway in the settings and any gateway added through the filter will be allowed
if ( pmpro_getOption( "gateway", true ) == "custommix" ) {
	$valid_gateways = apply_filters( "pmpro_valid_gateways", array( "custommix", "instamojo" ) );
} else {
	$valid_gateways = apply_filters( "pmpro_valid_gateways", array( pmpro_getOption( "gateway", true ) ) );
}
?>
         
	<?php if(pmpro_getGateway() == "custommix" && empty($pmpro_review)  ) { ?> <!-- && true == apply_filters('pmpro_include_payment_option_for_paypal', true ) -->
			...
			<tr>
				<td>
					<div>
						<span class="gateway_paypal">
							<input type="radio" name="my_choice" value="instamojo" <?php if($gateway == "custommix") { ?>checked="checked"<?php } ?> />
							<a href="javascript:void(0);" class="pmpro_radio"><?php _e('Wallets and UPI', 'pmpro');?></a>
						</span>
                                    ...
                                    <img src="https://www.webtalkies.in/wp-content/uploads/2017/02/1ola-money.png" width="60px" alt="payment-image"/>
                                    <img src="https://www.webtalkies.in/wp-content/uploads/2017/02/1jiomoney.png" width="60px" alt="payment-image"/>
                                    <img src="https://www.webtalkies.in/wp-content/uploads/2017/02/1freecharge.png" width="60px" alt="payment-image"/>
                                    <img src="https://www.webtalkies.in/wp-content/uploads/2017/02/1UPI-payments.png" width="60px" alt="payment-image"/>
                                    </td>
			</tr>
                        <tr>
				<td>
					<div>
						<span class="gateway_paypal">
							<input type="radio" name="my_choice" value="instamojo" <?php if($gateway == "paytm") { ?>checked=" "<?php } ?> />
							<a href="javascript:void(0);" class="pmpro_radio"><?php _e('Credit Cards, Net Banking', 'pmpro');?></a>
						</span>
                                               
					</div>
				</td>
                                    <td>
                                        <img src="https://www.webtalkies.in/wp-content/uploads/2017/02/1visa-250x100.png" width="60px" alt="payment-image"/> 
                                        <img src="https://www.webtalkies.in/wp-content/uploads/2017/02/1mastercard.png" width="60px" alt="payment-image"/>
                                        <img src="https://www.webtalkies.in/wp-content/uploads/2017/02/1icici-bank_logo.jpg" width="60px" alt="payment-image"/>
                                        <img src="https://www.webtalkies.in/wp-content/uploads/2017/02/1hdfc-bank.png" width="60px" alt="payment-image"/>
				...
				<td>
					<div>
							...
							<input type="radio" name="my_choice" value="paytm" <?php if($gateway == "paytm") { ?>checked=" "<?php } ?> />
							<a href="javascript:void(0);" class="pmpro_radio"><?php _e('Pay with PayTM', 'pmpro');?></a>
						</span>
					</div>
				</td>
                                <td><img src="https://www.webtalkies.in/wp-content/uploads/2017/02/1paytmrecharge.png" width="60px" alt="payment-image"/></td>
			</tr>
				...
				<td>
					<div>
                                               	<span class="gateway_paypalexpress">
							<input type="radio" name="my_choice" value="fortumo" <?php if($gateway == "paytm") { ?>checked=" "<?php } ?> />
							<a href="javascript:void(0);" class="pmpro_radio"><?php _e('Add in mobile bill', 'pmpro');?></a>
						</span>
					</div>
				</td>
                                <td><img src="https://www.webtalkies.in/wp-content/uploads/2016/01/fortumo-mobile-pay.jpeg" width="90px" alt="payment-image"/></td>
			</tr>
...
<?php
    ...
    echo 'Please register or login to proceed.';
